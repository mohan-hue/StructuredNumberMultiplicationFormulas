...
# 8. Relationship with NTT / FFT

## 8.1 What Is the Role of NTT / FFT?

NTT / FFT is a general-purpose method for fast large-integer multiplication or polynomial convolution.

It is suitable for:

```text
very large dense integers without obvious special structure
```

When the numbers are large enough, NTT / FFT is usually much faster than ordinary long multiplication.

---

## 8.2 Can Prefix/Suffix Formulas Replace NTT?

Usually, no.

Prefix/suffix formulas are:
...
```text
general-purpose fast multiplication
```
...
If there is a long prefix, long suffix, squaring case, or closeness to 2^n, use the special formula.
Otherwise, use NTT / FFT.
```

---

## 8.3 Recommended Process

```text
multiply(A, C):

1. If A == C:
      use a squaring algorithm

2. Check whether the numbers are close to 10^n or 2^n:
      if yes, use a special expansion

3. Check for a common prefix:
      if the common prefix is long, use the prefix formula

4. Check for a common suffix:
      if the common suffix is long, use the suffix formula

5. If both prefix and suffix are long:
      use the combined formula, and compute the middle part using ordinary multiplication or NTT

6. If none of the above applies:
      use Karatsuba / Toom-Cook / NTT / FFT
```

---

## 8.4 How to Judge Whether It Is Worth Using

Suppose both numbers have length `n`.

Let the common prefix length be `p`.

Let the common suffix length be `s`.

The length of the different middle part is approximately:
...
then the two numbers are mostly identical and differ only in a small middle part. In this case, the combined formula may be useful.
...
then the common structure is too small, and using NTT / FFT directly is usually better.

A simple rule of thumb:

| Situation | Suggestion |
|---|---|
| only one or two digits are the same | not worth decomposing |
| common part is below 5% | usually not worth decomposing |
| common part is around 20% | needs testing |
| common part is above 70% | likely worth trying |
| the different middle part is very short | very worth trying |
| the two numbers are exactly the same | use squaring |

---

# 9. Common Mistakes

## 9.1 Thinking That Sharing the First Digit Is Always Useful
...
## 9.2 Thinking That All Cross Terms Can Be Removed
...
## 9.3 Applying the Prefix Formula Without Alignment
...
## 9.4 The Suffix Must Be Consecutive from the Right
